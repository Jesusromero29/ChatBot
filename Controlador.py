    ...
    if (containsGreeting || (!conversationActive && !waitingForSelection)) {
        if (!conversationActive && !containsGreeting && !waitingForSelection) {
            ...
            if (!conversationActive && !waitingForSelection) {
                client.sendMessage(message.from, "Hola, soy tu periodista virtual, dime qué noticias, artículos o información deseas conocer esta vez");
            }
...
function closeConversation() {
    if (conversationActive) {
        ...
        if (lastMessage && client && conversationTimeout) {
            ...
            client.sendMessage(lastMessage.from, 'Bien, si no necesitas más nada por ahora, ya sabes que estaré aquí para traerte información cuando la necesites')
                .then(() => {
        ...
        if (lastMessage) {
            client.sendMessage(lastMessage.from, 'Parece que te has ido, nuestra conversación ha finalizado. Si necesitas más información, no dudes en escribirme.');
        }
    }
}
...
client.on('message', async message => {
    ...
    if (waitingForSelection && !isNaN(selectedOption)) {
        if (selectedOption === 6) {
            client.sendMessage(message.from, 'No hay problema, dime qué otra información puedo traerte.')
                .then(() => {
                    waitingForSelection = false;
                    clearTimeout(handleNewsTimeout);
                    return;
